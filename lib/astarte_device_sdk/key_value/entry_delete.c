...
static astarte_result_t update_shifted_entry_neighbors(
    struct nvs_fs *nvs_fs, const struct astarte_key_value_entry_header *header, uint16_t hole_id);
...
astarte_result_t astarte_key_value_entry_delete(struct nvs_fs *nvs_fs, uint16_t idx)
{
    ...
    ares = astarte_key_value_entry_intent_write(nvs_fs, ASTARTE_KEY_VALUE_ENTRY_INTENT_SHIFTING,
        idx, ASTARTE_KEY_VALUE_ENTRY_NULL_ID, ASTARTE_KEY_VALUE_ENTRY_NULL_ID);
    if (ares != ASTARTE_RESULT_OK) {
        return ares;
    }

    ares = astarte_key_value_entry_delete_resume_shift(nvs_fs, idx);
    if (ares != ASTARTE_RESULT_OK) {
        return ares;
    }
    ...
    struct nvs_fs *nvs_fs, uint16_t hole_id)
{
    ...
    while (curr_id != hole_id) {
        ...
        if (shift_performed) {
            ...
            ares = astarte_key_value_entry_intent_write(nvs_fs,
    ...
    struct nvs_fs *nvs_fs, uint16_t idx, uint16_t hole_id, bool *shift_performed)
{
    ...
    ares = astarte_key_value_entry_header_read(nvs_fs, source_id, &header, &raw_entry_size);
    if (ares != ASTARTE_RESULT_OK) {
        if (ares != ASTARTE_RESULT_NOT_FOUND) {
    ...
    uint16_t natural_hash = astarte_key_value_entry_hash_generate(header.namespace, header.key);
    uint16_t d_curr = (source_id >= natural_hash)
        ? (source_id - natural_hash)
        : (ASTARTE_KEY_VALUE_ENTRY_MAX_USABLE_ID - natural_hash + source_id + 1);
    uint16_t d_hole = (hole_id >= natural_hash)
        ? (hole_id - natural_hash)
        : (ASTARTE_KEY_VALUE_ENTRY_MAX_USABLE_ID - natural_hash + hole_id + 1);
    ...
    struct nvs_fs *nvs_fs, const struct astarte_key_value_entry_header *header, uint16_t hole_id)
{
    ...
    if (header->fixed_header.prev_id != ASTARTE_KEY_VALUE_ENTRY_NULL_ID) {
        ares = astarte_key_value_entry_list_update_next_id(
            nvs_fs, header->fixed_header.prev_id, hole_id);
        if (ares != ASTARTE_RESULT_OK) {
        ...
        ares = astarte_key_value_entry_list_write_head_and_tail_ids(nvs_fs, hole_id, tail_id);
        if (ares != ASTARTE_RESULT_OK) {
    ...
    if (header->fixed_header.next_id != ASTARTE_KEY_VALUE_ENTRY_NULL_ID) {
        ares = astarte_key_value_entry_list_update_prev_id(
            nvs_fs, header->fixed_header.next_id, hole_id);
        if (ares != ASTARTE_RESULT_OK) {
        ...
        ares = astarte_key_value_entry_list_write_head_and_tail_ids(nvs_fs, head_id, hole_id);
        if (ares != ASTARTE_RESULT_OK) {
            ASTARTE_LOG_ERR("Failed in writing head and tail IDs");
            return ares;
        }
...
static astarte_result_t delete_and_unlink_single_entry(struct nvs_fs *nvs_fs, uint16_t idx)
{
    ...
    ares = astarte_key_value_entry_intent_write(
        nvs_fs, ASTARTE_KEY_VALUE_ENTRY_INTENT_DELETING, idx, prev_id, next_id);
    if (ares != ASTARTE_RESULT_OK) {
        return ares;
    }
