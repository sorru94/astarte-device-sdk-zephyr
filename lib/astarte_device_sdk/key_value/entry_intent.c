...
static astarte_result_t resolve_insert_intent(
    struct nvs_fs *nvs_fs, struct astarte_key_value_entry_intent *intent);
    ...
    uint16_t affected_id_2)
{
    ...
    ssize_t ret = nvs_write(nvs_fs, ASTARTE_KEY_VALUE_ENTRY_INTENT_ID, &intent, sizeof(intent));
    if (ret < 0) {
...
astarte_result_t astarte_key_value_entry_intent_resolve(struct nvs_fs *nvs_fs)
{
    ...
    ssize_t ret = nvs_read(nvs_fs, ASTARTE_KEY_VALUE_ENTRY_INTENT_ID, &intent, sizeof(intent));
    ...
    if (intent.state == ASTARTE_KEY_VALUE_ENTRY_INTENT_INSERTING) {
        ares = resolve_insert_intent(nvs_fs, &intent);
        ...
        ares = resolve_delete_intent(nvs_fs, &intent);
    }
    ...
    if (intent.state == ASTARTE_KEY_VALUE_ENTRY_INTENT_SHIFTING) {
        ares = resolve_shift_intent(nvs_fs, &intent);
    }
    ...
    // Clear the intent block now that we have cleaned up the NVS state
    return astarte_key_value_entry_intent_clear(nvs_fs);
}
...
{
    /*
     * NVS Flash Write Operations Breakdown
     *
     ...
     * list pointers.
     * - Write 1: nvs_write writes the actual updated serialized payload to the target ID.
     *
    ...
    ares = astarte_key_value_entry_list_read_head_and_tail_ids(nvs_fs, &head_id, &tail_id);
    if (ares != ASTARTE_RESULT_OK) {
        return ares;
    }
    ...
    uint16_t prev_id = intent->affected_id_1;
    ...
    if (prev_id != ASTARTE_KEY_VALUE_ENTRY_NULL_ID) {
        ares = astarte_key_value_entry_list_update_next_id(
            nvs_fs, prev_id, ASTARTE_KEY_VALUE_ENTRY_NULL_ID);
        if (ares != ASTARTE_RESULT_OK) {
    ...
    ssize_t del_ret = nvs_delete(nvs_fs, intent->target_id);
    if (del_ret < 0) {
    ...
    struct nvs_fs *nvs_fs, struct astarte_key_value_entry_intent *intent)
{
     ...
     *            and tail.
     * - Write 2: nvs_delete removes the serialized payload.
     *
     ...
     * - Write 3: nvs_delete removes the serialized payload.
     *
    ...
    // Check if the target entry has already been successfully deleted from NVS
    astarte_result_t ares = ASTARTE_RESULT_OK;
    struct astarte_key_value_entry_header_fixed check_header = { 0 };
    size_t check_size = 0;
    ares = astarte_key_value_entry_header_read_fixed(
        nvs_fs, intent->target_id, &check_header, &check_size);
    ...
    if (ares == ASTARTE_RESULT_NOT_FOUND) {
        ...
        uint16_t prev_id = intent->affected_id_1;
        uint16_t next_id = intent->affected_id_2;

        // Repair prev_id's next pointer (if it was no head)
        if (prev_id != ASTARTE_KEY_VALUE_ENTRY_NULL_ID) {
            ares = astarte_key_value_entry_list_update_next_id(nvs_fs, prev_id, next_id);
            if (ares != ASTARTE_RESULT_OK) {
        ...
        if (next_id != ASTARTE_KEY_VALUE_ENTRY_NULL_ID) {
            ares = astarte_key_value_entry_list_update_prev_id(nvs_fs, next_id, prev_id);
            if (ares != ASTARTE_RESULT_OK) {
        ...
        if (changed) {
            ares = astarte_key_value_entry_list_write_head_and_tail_ids(nvs_fs, head_id, tail_id);
            if (ares != ASTARTE_RESULT_OK) {
        ...
        // Safely delete the orphaned payload physically from NVS
        ssize_t del_ret = nvs_delete(nvs_fs, intent->target_id);
        if (del_ret < 0) {
            return ASTARTE_RESULT_NVS_ERROR;
        }
    ...
    return astarte_key_value_entry_intent_write(nvs_fs, intent->state, intent->target_id,
    ...
    struct nvs_fs *nvs_fs, struct astarte_key_value_entry_intent *intent)
{
    ...
    uint16_t current_hole = intent->target_id;
    struct astarte_key_value_entry_header_fixed dummy_header = { 0 };
    size_t dummy_size = 0;

    while (true) {
        ares = astarte_key_value_entry_header_read_fixed(
            nvs_fs, current_hole, &dummy_header, &dummy_size);
        if (ares == ASTARTE_RESULT_NOT_FOUND) {
            break;
        }
        if (ares != ASTARTE_RESULT_OK) {
    ...
    return astarte_key_value_entry_delete_resume_shift(nvs_fs, current_hole);
}
