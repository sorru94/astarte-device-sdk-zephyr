...
[Astarte in 5 minutes](https://docs.astarte-platform.org/astarte/latest/010-astarte_in_5_minutes.html)
tutorial.
The tutorial guides you through setting up an Astarte instance, connecting a simulated device, and
streaming some data. This application will replace the *Astarte in 5 minutes* simulated device with
a real device.

Following *Astarte in 5 minutes* install Astarte and create a realm named *test*.
The steps that follow will be slightly different for this application compared to the standard
tutorial.
...
Use the same method described in the *Install interfaces* step of *Astarte in 5 minutes* to
install this application interfaces.

### 3.3 Step 3

An Astarte instance can have many devices connected to it at the same time. Where each device is
uniquely identified within a realm by a [device ID](https://docs.astarte-platform.org/astarte/latest/010-design_principles.html#device-id).
...
Using the `astartectl` tool used in *Astarte in 5 minutes* a device can be registered manually as
follows.
```
astartectl pairing agent register DEVICE_ID
```
Alternatively, a registration JWT can be obtained with the following.
```
astartectl utils gen-jwt pairing -k test_private.pem
```
Both commands should be run in the Astarte folder where the Astarte instance lives.
...
CONFIG_ASTARTE_DEVICE_SDK_PAIRING_JWT="PAIRING_JWT" # Required using on board registration

# Sample configuration
CONFIG_DEVICE_ID="DEVICE_ID"
CONFIG_CREDENTIAL_SECRET="CREDENTIAL_SECRET" # Required using off board registration
...
- `CREDENTIAL_SECRET` is the credential secret for the device obtained during off-board
  ...
  manually on Astarte.
- `PAIRING_JWT` is the pairing JWT for the device to be used for on-board registration. This option
  should be set only when the registration of the device takes place on-board.
