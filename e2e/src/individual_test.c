 ...
 */
static void server_send_individual_data(const e2e_interface_data_array *server_interfaces_data);
/**
 * @brief Helper function used to check the data the got transmitted in function
 * #transmit_datastream_data.
 */
static void check_individual_data(const e2e_interface_data_array *device_interfaces_data);
...
e2e_device_config_t get_individual_test_config()
{
    return (e2e_device_config_t){
        ...
        .server_send = server_send_individual_data,
        .check_server_received = check_individual_data,
...
static e2e_test_data_t setup_test_data()
{
    ...
    e2e_mapping_data_array device_mappings = DYNAMIC_ARRAY(e2e_individual_data_t, ((e2e_individual_data_t[]){
        ...
        { .endpoint = datetimearray_endpoint,
                ...
                .individual = astarte_individual_from_datetime_array((int64_t *) utils_unix_time_array_data,
        ...
        { .endpoint = stringarray_endpoint,
                ...
                .individual = astarte_individual_from_string_array((const char **) utils_string_array_data,
                    ARRAY_SIZE(utils_string_array_data)) }
    }));
    // the pointer stored in this array (endpoint and buffers of astarte values)
    // need to be statically allocated and need be valid until the test data is deallocated
    e2e_mapping_data_array server_mappings = DYNAMIC_ARRAY(e2e_individual_data_t,
    ...
    return (e2e_test_data_t){ .device_sent
        = DYNAMIC_ARRAY(e2e_interface_data_t,
            ((e2e_interface_data_t[]){
                e2e_interface_from_interface_individual(
                    &org_astarte_platform_zephyr_e2etest_DeviceDatastream, device_mappings),
            })),
        .server_sent = DYNAMIC_ARRAY(e2e_interface_data_t,
            ((e2e_interface_data_t[]){
                e2e_interface_from_interface_individual(
                    &org_astarte_platform_zephyr_e2etest_ServerDatastream, server_mappings),
            })) };
}

static void check_individual_data(const e2e_interface_data_array *interfaces_data)
{
    (void) interfaces_data;
    // TODO connect to appengine apis and check data sent
