...
class MyBuilder:
    ...
    missing_links = [
        ...
        # ("astarte_device_connection_on_disconnected_handler", "disconnection_callback", 0),
        ("astarte_device_connection_poll", "connection_callback", 0),
        ("astarte_device_rx_on_incoming_handler", "datastream_object_callback", 3),
        ("astarte_device_rx_on_incoming_handler", "datastream_individual_callback", 2),
        ("astarte_device_rx_on_incoming_handler", "set_property_callback", 1),
        ("astarte_device_rx_on_incoming_handler", "unset_property_callback", 0),
    ]
...
def find_all_blx_instructions(collector):

    symbols_with_fn_ptr_call = []
    for _, symbol in collector.symbols.items():
        if "astarte-device-sdk-zephyr" in str(symbol["path"]):
...
if __name__ == '__main__':
    ...
    astarte_device_sdk_path = script_path.parent
    zephyr_project_path = astarte_device_sdk_path.parent
    # NB: The build folder is expected in the project directory
    build_dir_path = zephyr_project_path.joinpath("build")
    ...
    elf_file = build_dir_path.joinpath("zephyr", "zephyr.elf")
    ...
    find_all_blx_instructions(builder.collector)

    generate_graph(builder.collector, "device_rx_thread_entry_point", script_path.joinpath("generated.mermaid"))
