...
ZTEST(astarte_device_sdk_bson, test_bson_deserializer_check_validity)
{
    ...
    zassert_false(bson_deserializer_check_validity(empty_buffer, sizeof(empty_buffer)));

    uint8_t minimal_doc[] = { 0x8, 0x0, 0x0, 0x0, 0x8, 0x0, 0x1, 0x0 };
    zassert_false(bson_deserializer_check_validity(minimal_doc, sizeof(minimal_doc) - 1));

    uint8_t empty_doc_incorrect_termination[] = { 0x05, 0x00, 0x00, 0x00, 0x01 };
    zassert_false(bson_deserializer_check_validity(
    ...
    zassert_false(bson_deserializer_check_validity(too_small_doc, sizeof(too_small_doc)));
    ...
    zassert_true(bson_deserializer_check_validity(minimal_doc, sizeof(minimal_doc)));
...
ZTEST(astarte_device_sdk_bson, test_bson_deserializer_empty_bson_document)
{
    ...
    zassert_equal(ASTARTE_ERR_NOT_FOUND, bson_deserializer_first_element(doc, &element),
        "astarte err returned != from ASTARTE_ERR_NOT_FOUND");
}
...
ZTEST(astarte_device_sdk_bson, test_bson_deserializer_complete_bson_document)
{
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_first_element(doc, &element_d));

    zassert_equal(BSON_TYPE_DOUBLE, element_d.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_d, &element_s));

    zassert_equal(BSON_TYPE_STRING, element_s.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_s, &element_doc));

    zassert_equal(BSON_TYPE_DOCUMENT, element_doc.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_first_element(subdocument, &subelement_int32));

    zassert_equal(BSON_TYPE_INT32, subelement_int32.type);
    ...
    zassert_equal(BSON_TYPE_BOOLEAN, subelement_bool.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_doc, &element_arr));

    zassert_equal(BSON_TYPE_ARRAY, element_arr.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_first_element(subdoc_arr, &subelement_arr_1));

    zassert_equal(BSON_TYPE_INT32, subelement_arr_1.type);
    ...
    zassert_equal(BSON_TYPE_DOUBLE, subelement_arr_2.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_arr, &element_bin));

    zassert_equal(BSON_TYPE_BINARY, element_bin.type);
    ...
    zassert_equal(
        ASTARTE_OK, bson_deserializer_next_element(doc, element_bin, &element_bool_false));

    zassert_equal(BSON_TYPE_BOOLEAN, element_bool_false.type);
    ...
    zassert_equal(
        ASTARTE_OK, bson_deserializer_next_element(doc, element_bool_false, &element_bool_true));

    zassert_equal(BSON_TYPE_BOOLEAN, element_bool_true.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_bool_true, &element_utc));

    zassert_equal(BSON_TYPE_DATETIME, element_utc.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_utc, &element_int32));

    zassert_equal(BSON_TYPE_INT32, element_int32.type);
    ...
    zassert_equal(ASTARTE_OK, bson_deserializer_next_element(doc, element_int32, &element_int64));

    zassert_equal(BSON_TYPE_INT64, element_int64.type);
    ...
    zassert_equal(ASTARTE_ERR_NOT_FOUND,
...
ZTEST(astarte_device_sdk_bson, test_bson_deserializer_bson_document_lookup)
{
    ...
    zassert_equal(
        ASTARTE_OK, bson_deserializer_element_lookup(doc, "element double", &element_double));

    zassert_equal(BSON_TYPE_DOUBLE, element_double.type);
    ...
    zassert_equal(
        ASTARTE_OK, bson_deserializer_element_lookup(doc, "element bool true", &element_bool));

    zassert_equal(BSON_TYPE_BOOLEAN, element_bool.type);
    ...
    bson_element_t element_int64;
    zassert_equal(
        ASTARTE_OK, bson_deserializer_element_lookup(doc, "element int64", &element_int64));
    ...
    zassert_equal(
        ASTARTE_ERR_NOT_FOUND, bson_deserializer_element_lookup(doc, "foo", &element_foo));
